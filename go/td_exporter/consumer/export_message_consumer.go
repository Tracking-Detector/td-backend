...
func (c *ExportMessageConsumer) Consume() {
	...
	msgs, err := c.queueAdapter.Consume(
		config.EnvExportQueueName(), // queue name
		"ExportConsumer",
		true,  // auto-ack
		false, // exclusive
		false, // no-local
	...
	log.Println("Export ConsumerService started. Waiting for message...")
...
func (c *ExportMessageConsumer) handleMessage(msg []byte) {
	...
	jobValue, err := message.DeserializeJob(string(msg))
	if err != nil {
	...
	go func() {
		start := time.Now()
		run, err := c.exportRunService.Save(c.cancelContext, &model.ExportRun{
