...
func (j *DatasetMetricJob) Execute() {
	...
	for _, dataset := range datasets {
		dataset.Metrics = &model.DataSetMetrics{
			Total:         0,
			ReducerMetric: make([]*model.ReducerMetric, 0),
		}
	}
loop:
	for {
...
func (j *DatasetMetricJob) FindMetricForLabel(metrics []*model.ReducerMetric, label string) *model.ReducerMetric {
